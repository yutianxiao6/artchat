(function () {
  ...
  async function safeApiJson(res, context = "api") {
    ...
    try {
      ...
      console.error(`[safeApiJson][${context}] invalid json`, {
        status: res.status,
        contentType,
        preview: previewText(text, 400),
      });
      if (/<!doctype html/i.test(text) || /<html/i.test(text)) {
        throw new Error(`接口返回了 HTML 页面，不是 JSON。通常是请求没有被安卓本地 API 拦截，或者当前包里的前端资源不是最新版本。状态码: ${res.status}`);
      }
      throw new Error(`接口返回的不是合法 JSON。状态码: ${res.status}，内容预览: ${previewText(text, 120)}`);
    }
  }

  window.safeApiJson = safeApiJson;

  let chatModuleReady = false;
  let mainModuleReady = false;

  window.__chatModuleReady = function () {
    chatModuleReady = true;
    tryReleaseAppBoot();
  };

  function tryReleaseAppBoot() {
    if (chatModuleReady && mainModuleReady) {
      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          if (typeof window.__releaseAppBoot === "function") {
            window.__releaseAppBoot();
          }
        });
      });
    }
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    ...
    activateTab("chat", { force: true, noReApply: true });
    bindSelects();
  ...
  async function loadAllConfigs() {
    try {
      ...
      const result = await safeApiJson(res, "loadAllConfigs");
      if (result.code === 0) {
