...
function normalizeImageItems(raw) {
  ...
  async function handleImage(request) {
    ...
    if (Array.isArray(body.image_base64_list) && body.image_base64_list.length) payload.image_base64_list = body.image_base64_list;
  ...
  async function handleCanvasAssets(request) {
    ...
    if (method === 'POST') {
      ...
      return ok({ assetLibrary: state.assetLibrary, categories: state.categories }, '素材已保存');
    }
    if (method === 'DELETE') {
      const body = await getJsonBody(request);
