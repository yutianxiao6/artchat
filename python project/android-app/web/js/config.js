(function () {
  ...
  function injectConfigStyles() {
    ...
    style.textContent = `
      .config-model-preset-group{padding:10px;border:1px solid rgba(226,232,240,.95);border-radius:14px;background:linear-gradient(180deg,#ffffff,#f8fafc)}
      .config-model-preview{min-height:40px;display:flex;align-items:center}
      .config-model-preview-empty{font-size:11px;color:#94a3b8;font-weight:600}
      #config-modal .table td .oc-model-chip,#config .table td .oc-model-chip{min-width:180px;max-width:100%;background:rgba(248,250,252,.92);border-color:rgba(226,232,240,.95)}
      #config-modal .table-container,#config .table-container{border:1px solid rgba(226,232,240,.95);border-radius:14px;background:#fff}
      #config-modal .table td,#config .table td{vertical-align:middle}
      #config-modal .card-body.modal-stack{gap:12px}
      #config-modal .modal-card{background:#f8fafc}
      #config-modal .card-header,#config .card-header{padding:12px 14px;border-bottom:1px solid #e5e7eb;background:rgba(255,255,255,.96)}
      #config-modal .card-title,#config .card-title{font-size:13px;gap:6px}
      #config-modal .card-body,#config .card-body{padding:12px}
      #config-modal .form-grid,#config .form-grid{grid-template-columns:1fr;gap:10px;margin-bottom:10px}
      #config-modal .form-group,#config .form-group{margin-bottom:10px}
      #config-modal .form-label,#config .form-label{font-size:11px;margin-bottom:4px;color:#64748b;font-weight:600}
      #config-modal .form-input,#config-modal .form-select,#config-modal .form-textarea,#config .form-input,#config .form-select,#config .form-textarea{background:#fff;border:1px solid #dbe2ea;border-radius:12px;color:#0f172a;font-size:12px;padding:10px 12px}
      #config-modal .form-textarea,#config .form-textarea{min-height:72px}
      #config-modal .btn-group,#config .btn-group{display:grid;grid-template-columns:1fr 1fr;gap:8px}
      #config-modal .btn,#config .btn{min-height:34px;border-radius:10px;font-size:11px;padding:0 10px;justify-content:center}
      #config-modal .table,#config .table{min-width:0;font-size:12px}
      #config-modal thead,#config thead{display:none}
      #config-modal tbody tr,#config tbody tr{display:block;border:none;background:#fff;border:1px solid #e2e8f0;border-radius:14px;margin-bottom:10px;padding:10px 12px;box-shadow:0 6px 16px rgba(15,23,42,.04)}
      #config-modal tbody td,#config tbody td{display:block;padding:2px 0;border:none;text-align:left!important}
      #config-modal tbody td:nth-child(1),#config tbody td:nth-child(1){font-size:13px;font-weight:700;color:#0f172a;margin-bottom:4px}
      #config-modal tbody td:nth-child(2),#config tbody td:nth-child(2){font-size:11px;color:#64748b;margin-bottom:6px}
      #config-modal tbody td:nth-child(4),#config tbody td:nth-child(4){margin-top:8px}
      #config-modal .table-action,#config .table-action{justify-content:flex-start;gap:8px}
      #config-modal .table-btn,#config .table-btn{font-size:12px;padding:6px 8px;border-radius:8px;background:#f8fafc;border:1px solid #e2e8f0}
    `;
