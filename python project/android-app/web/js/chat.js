...
async function readSmartFile(file) {
  ...
  if (canReadAsText) {
    ...
    const finalText = text ? text.slice(0, 20000) : `[文件已上传：${file.name}，但未能读取到文本内容。]`;
  ...
  const placeholder = isPdf
    ? `[PDF 文件已上传：${file.name}。当前版本暂未解析 PDF 正文，请结合文件名和问题回答。]`
    : `[Word 文件已上传：${file.name}。当前版本暂未解析 DOC/DOCX 正文，请结合文件名和问题回答。]`;
  console.log("[chat][file] placeholder only", { name: file.name, ext, size: file.size, extractedLength: placeholder.length });
  return { filename: file.name, content_type: file.type || (isPdf ? "application/pdf" : "application/vnd.openxmlformats-officedocument.wordprocessingml.document"), text_content: placeholder, size: file.size };
}
...
function bindEvents() {
  ...
  if (plusMenu && !plusMenu._bound) {
    plusMenu.addEventListener("click", (e) => {
      const pick = e.target.closest("[data-chat-pick]");
      if (!pick || !fileInput) return;
      const kind = pick.getAttribute("data-chat-pick");
      fileInput.setAttribute("accept", kind === "image" ? "image/*" : "image/*,.txt,.md,.json,.csv,.py,.js,.ts,.html,.css,.xml,.yaml,.yml,.log,.ini,.cfg,.sql,.java,.kt,.go,.rs,.sh,.pdf,.doc,.docx");
      plusMenu.classList.add("hidden");
      fileInput.click();
    });
    plusMenu._bound = true;
  }
  if (!document.body._chatPlusDismissBound) {
    document.body.addEventListener("click", () => document.getElementById("chat-plus-menu")?.classList.add("hidden"));
    document.body._chatPlusDismissBound = true;
  }
  ...
  tab.innerHTML = `
    <div class="oc-chat-shell">
      ...
      <section class="oc-main">
        ...
        <input type="file" class="oc-hidden-file-input" id="chat-file-input" multiple accept="image/*,image/*,.txt,.md,.json,.csv,.py,.js,.ts,.html,.css,.xml,.yaml,.yml,.log,.ini,.cfg,.sql,.java,.kt,.go,.rs,.sh,.pdf,.doc,.docx">
        ...
        <div class="oc-input-wrap">
          <div class="oc-input-box">
            <div class="oc-plus-wrap">
              ...
              <div class="oc-plus-menu hidden" id="chat-plus-menu">
                <button type="button" data-chat-pick="image">上传图片</button>
                <button type="button" data-chat-pick="file">上传文件</button>
              </div>
            </div>
