...
【核心写作原则】
- 每格只写一个核心动作，不要堆叠多个动作（模型无法同时执行多个复杂动作）
- 动作用具体动词+速度副词（缓缓/轻轻/渐渐/微微/突然），不要泛写"站着""走着"
- 镜头运动从高频词选择：静止/缓推/拉远/环绕/跟拍/横移/上摇/下摇/手持/俯冲
- 景别明确标注：大远景/远景/全景/中景/中近景/近景/特写/大特写
- 不要堆砌抽象形容词（"美丽的""唯美的""高级感"对模型无效）
- 因为有分镜图作为参考，画面构成已由图片决定，重心放在【动作过程】和【镜头运动】上
...
每个时间段按三层结构描述：
1. 【镜头层】景别 + 镜头运动方式 + 速度（从高频词选择）
2. 【主体层】人物的一个核心动作 + 表情/姿态变化（根据图片中看到的内容，带速度副词）
3. 【画面层】光线方向、场景环境关键物体（简要，不要过度描述图中已有的静态信息）
...
- 景别明确标注：大远景/远景/全景/中景/中近景/近景/特写/大特写
- 不要堆砌抽象形容词（"美丽的""唯美的""高级感"对模型无效）
- 因为有分镜图参考，画面构成已由图片决定，重心放在【动作过程】和【镜头运动】上
...
每个时间段按三层结构描述：
1. 【镜头层】景别 + 镜头运动方式 + 速度（从高频词选择，如"中景，镜头缓缓推进"）
2. 【主体层】人物的一个核心动作 + 表情/姿态变化 + 速度副词（如"她缓缓转过头，微微一笑"）
3. 【画面层】光线方向、环境氛围细节（简要，不过度描述图中已有的静态信息）
4. 如果剧本中有台词或对白，在对应时间段内写出台词内容（用引号标注），注明说话人和语气
...
【提示词写作核心原则】
- 视频生成模型在训练数据分布里匹配视频片段，提示词越像训练标注的语法效果越稳定
- 每个镜头只写一个核心动作，不要堆叠多个复杂动作
- 动作用具体动词+速度副词（缓缓/轻轻/渐渐/微微/突然），不可泛写"站着""走着"
- 镜头运动从高频词选择：静止/缓推/拉远/环绕/跟拍/横移/上摇/下摇/手持/俯冲
- 不要使用对模型无效的抽象词（"高级感""氛围感""史诗级""唯美"等训练标注中不存在的词）
- 光线用具体词：黄金时刻/蓝色时刻/侧光/逆光/霓虹光/柔和窗光

## 输出格式（四个段落，按顺序输出，不可省略任何段落）

### 视觉风格
综合 overview 的 visual_style 和各 shot 的 style_cue，写出整体视觉风格：色调倾向、光影质感、画幅比例、节奏（{'快切' if avg_shot_sec < 1.2 else ('中速' if avg_shot_sec < 2.5 else '长镜头')}）、参考风格流派。≤80字，一段写完。用具体的风格词（如"青橙对比色调""胶片颗粒质感""16:9宽画幅"），不要用抽象感受词。
...
3) **精确动作**：一个核心动作+速度副词，具体动作过程、肢体姿势变化、表情变化、与物体的交互方式（手指怎么动、手腕怎么转等细节），不可泛写"站着""走着"
4) **场景环境**：光线方向与色调（用具体光线词）、关键物体及其位置、空间类型（参考 scene_id → 场景介绍）
5) **镜头运动**：运镜方式（静止/缓推/拉远/环绕/跟拍/横移/上摇/下摇/手持）+ 速度（缓慢/匀速/快速）+ 景别（特写/近景/中景/全景/远景），从 shot.camera 和 shot.composition 提取
...
async def gen_reverse_video_prompt_chat(body: dict):
    ...
    system_prompt = f"""你是AI视频生成提示词修改专家，精通视频生成模型的工作原理。用户对当前的视频提示词不满意，需要根据反馈进行修改。

核心原则：视频生成模型在训练数据分布里匹配视频片段，提示词越像训练标注的语法效果越稳定。
...
7) 每个镜头只保留一个核心动作，动作用具体动词+速度副词（缓缓/轻轻/渐渐/微微）
8) 镜头运动从高频词选择：静止/缓推/拉远/环绕/跟拍/横移/上摇/下摇/手持
9) 删除对模型无效的抽象词（"高级感""氛围感""史诗级""唯美"等）
10) 整篇 ≤3000字
11) 不要在输出中描述字幕、水印、台标等叠加元素
