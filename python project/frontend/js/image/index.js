(function () {
  ...
  function injectCanvasStyles() {
    if (document.getElementById("image-canvas-style-link")) return;
    const link = document.createElement("link");
    link.id = "image-canvas-style-link";
    link.rel = "stylesheet";
    link.href = `./css/image-canvas.css?v=20260426-0006`;
    document.head.appendChild(link);

    const patch = document.createElement("style");
    patch.id = "image-canvas-style-runtime-patch";
    patch.textContent = `
      .edge-dom-layer{display:none !important;}
      .edge-dom-hit{display:none !important;}
    `;
    document.head.appendChild(patch);

    if (window.location.search.includes('debugCanvasHit=1')) {
      document.body.classList.add('debug-canvas-hit');
    }
  }
    ...
    tab.innerHTML = `
      <div class="canvas-shell">
        ...
        <section class="canvas-main">
          <div class="canvas-topbar"><div class="left"><span class="badge">双击创建节点</span><span class="badge">滚轮缩放</span><span class="badge">Ctrl+C / Ctrl+V / Delete</span><button class="btn btn-default" id="toggle-canvas-debug-btn">命中调试</button><span class="badge" id="canvas-debug-badge" style="display:none;background:rgba(239,68,68,.16);color:#fecaca;border:1px solid rgba(239,68,68,.24)">DEBUG HIT ON</span></div><div class="right"><button class="btn btn-default" id="quick-add-node-btn">新建节点</button><span class="badge" id="canvas-zoom-badge">100%</span><button class="btn btn-default" id="reset-canvas-btn">清空画布</button></div></div>
  ...
  function bindWorkbenchEvents() {
    ...
    root.addEventListener("mousedown", (event) => {
      ...
      const nodeEdgeStart = target.closest(".node");
      if (nodeEdgeStart && !target.closest('.port-handle, textarea, input, select, button, a')) {
        const nodeId = nodeEdgeStart.getAttribute("data-node-id");
        const rect = nodeEdgeStart.getBoundingClientRect();
        const nearRightEdge = event.clientX >= rect.right - 28 && event.clientX <= rect.right + 24;
        if (nearRightEdge) {
          event.preventDefault();
    ...
    window.addEventListener("mousemove", (event) => {
      const hoverTarget = event.target instanceof HTMLElement ? event.target.closest("[data-edge-id]") : null;
      const nextHoverEdgeId = hoverTarget ? hoverTarget.getAttribute("data-edge-id") : null;
      if (STATE.hoverEdgeId !== nextHoverEdgeId) {
        STATE.hoverEdgeId = nextHoverEdgeId;
        if (!STATE.draggingNodeId && !STATE.connectionDrag && !STATE.selectionBox && !STATE.panning) renderCanvas();
      }
  ...
  function beginConnectionDrag(nodeId, clientX, clientY) {
    ...
    const startPort = document.querySelector(`.port-handle.output[data-node-id="${nodeId}"]`);
    let start = getPortPosition(node, "output");
    if (startPort) {
      const worldRect = document.getElementById("canvas-world")?.getBoundingClientRect();
      const portRect = startPort.getBoundingClientRect();
      if (worldRect && portRect) {
        start = {
    ...
    STATE.connectionDrag = { fromNodeId: nodeId, start, current: clientToCanvasPoint(clientX, clientY), targetNodeId: null };
    renderCanvas();
  }
  ...
  function findNearestInputPort(point, excludeNodeId) {
    ...
    for (const node of STATE.nodes) {
      ...
      const portEl = document.querySelector(`.port-handle.input[data-node-id="${node.id}"]`);
      let port = getPortPosition(node, "input");
      if (portEl) {
        const worldRect = document.getElementById("canvas-world")?.getBoundingClientRect();
...
function downloadImage(imageUrl) {
  ...
  function getNodeImageState(node) {
    const { linkedImageNodes, linkedImageNode, displayImageUrl, showingGeneratedImage, showingOwnReferenceImage, imageTitle } = getNodeImageState(node);
  ...
  function renderEdge(edge) {
    ...
    const stroke = selected ? "#f59e0b" : (hovered ? "#93c5fd" : "#60a5fa");
    const strokeWidth = selected ? 5.5 : (hovered ? 4.5 : 3.4);
    return `
      <path class="edge-hit" data-edge-id="${edge.id}" d="${d}"></path>
      <path class="edge-visible ${selected ? 'edge-selected' : ''} ${hovered ? 'edge-hovered' : ''}" data-edge-id="${edge.id}" d="${d}" stroke="${stroke}" stroke-width="${strokeWidth}"></path>
    `;
  }
