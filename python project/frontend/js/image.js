(function () {
  ...
  const GPT_IMAGE_SIZE_PRESETS = [
    { label: "1024×1024 正方形", value: "1024x1024" },
    { label: "1536×1024 横向", value: "1536x1024" },
    { label: "1024×1536 竖屏", value: "1024x1536" },
    { label: "2048×2048 2K正方形", value: "2048x2048" },
    { label: "2048×1152 2K横向", value: "2048x1152" },
    { label: "3840×2160 4K横向", value: "3840x2160" },
  ...
  async function runGenerateNode(nodeId) {
    ...
    const { width, height } = isGptImageModel(nodeCfg?.model_name)
      ? parseGptImageSize(node.gptImageSize)
      : resolveCanvasSize(node.resolution, node.ratio);
  ...
  function patchExistingNode(el, node) {
    ...
    const { displayImageUrl, linkedImageNodes, linkedImageNode, outputPreview } = getNodeImageState(node);
    ...
    if (imgWrap) {
      ...
      if (displayImageUrl) {
        ...
        if (!imgWrap.querySelector('.node-image-overlay')) {
          const overlayHtml = `<div class="node-image-overlay"><div class="node-image-toolbar"><button class="btn btn-default" type="button" data-image-preview="${displayImageUrl}">预览</button><button class="btn btn-default" type="button" data-save-image-to-library="${displayImageUrl}" data-image-title="${escapeHtml(imageTitle)}">加入素材库</button></div></div>`;
          imgWrap.insertAdjacentHTML('beforeend', overlayHtml);
        }
        if (!imgWrap.querySelector('.node-image-top-actions')) {
          const topActionsHtml = showingOwnReferenceImage
            ? `<div class="node-image-top-actions"><button class="btn btn-default" type="button" data-download-image="${displayImageUrl}"><i class="fa fa-download"></i> 下载</button><button class="btn btn-danger" type="button" data-remove-reference-image="${node.id}"><i class="fa fa-times"></i> 关闭</button></div>`
            : showingGeneratedImage
            ? `<div class="node-image-top-actions"><button class="btn btn-default" type="button" data-download-image="${displayImageUrl}"><i class="fa fa-download"></i> 下载</button><button class="btn btn-danger" type="button" data-clear-output-images="${node.id}"><i class="fa fa-times"></i> 关闭</button></div>`
            : `<div class="node-image-top-actions"><button class="btn btn-default" type="button" data-download-image="${displayImageUrl}"><i class="fa fa-download"></i> 下载</button></div>`;
          imgWrap.insertAdjacentHTML('afterbegin', topActionsHtml);
        }
...
function downloadImage(imageUrl) {
  ...
  function renderNode(node) {
    ...
    const currentModelCfg = getImageConfigById(node.modelId);
    const useGptImageSizes = isGptImageModel(currentModelCfg?.model_name);
    const resolutionOptions = RESOLUTION_PRESETS.map((size) => `<option value="${size}" ${size === (node.resolution || RESOLUTION_PRESETS[0]) ? "selected" : ""}>${size}</option>`).join("");
    const ratioOptions = RATIO_PRESETS.map((ratio) => `<option value="${ratio}" ${ratio === (node.ratio || RATIO_PRESETS[0]) ? "selected" : ""}>${ratio}</option>`).join("");
    const gptSizeOptions = GPT_IMAGE_SIZE_PRESETS.map((s) => `<option value="${s.value}" ${s.value === (node.gptImageSize || GPT_IMAGE_SIZE_PRESETS[0].value) ? "selected" : ""}>${s.label}</option>`).join("");
    const linkedImageNodes = getLinkedImageNodes(node.id);
    const linkedImageNode = linkedImageNodes[0] || null;
    const outputPreview = node.outputImages?.[0] || "";
    const ownReferenceImage = !outputPreview ? node.imageUrl : "";
    ...
    const previewHtml = displayImageUrl
      ? `<img src="${displayImageUrl}" alt="node-image" data-context-image="${displayImageUrl}" data-image-title="${escapeHtml(imageTitle)}">${topActions}<div class="node-image-overlay"><div class="node-image-toolbar"><button class="btn btn-default" type="button" data-image-preview="${displayImageUrl}">预览</button><button class="btn btn-default" type="button" data-save-image-to-library="${displayImageUrl}" data-image-title="${escapeHtml(imageTitle)}">加入素材库</button></div></div>`
      : `<div class="node-image-empty">${linkedImageNodes.length ? `已连接 ${linkedImageNodes.length} 个上游参考图，当前节点将使用它们生成。` : '上传一张参考图，或者把一个带图片的上游节点连到这里。'}</div>`;
  ...
  function persistCanvasStateDebounced() { if (_persistCanvasTimer) clearTimeout(_persistCanvasTimer); _persistCanvasTimer = setTimeout(() => { _persistCanvasTimer = null; persistCanvasState(); }, 600); }
  ...
  function parseGptImageSize(sizeValue) {
    const valid = GPT_IMAGE_SIZE_PRESETS.find((s) => s.value === sizeValue);
    const raw = valid ? valid.value : GPT_IMAGE_SIZE_PRESETS[0].value;
