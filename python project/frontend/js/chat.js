...
function enhanceCodeBlocks(container) {
  ...
  pres.forEach((pre) => {
    ...
    if (code) {
      ...
      if (match) {
        lang = match[1];
      } else if (classList.match(/hljs/)) {
        const langClass = classList.split(/\s+/).find(c => c !== 'hljs' && !c.startsWith('language-'));
        if (langClass) lang = langClass;
      }
    }
    ...
    header.innerHTML = `<span class="oc-code-lang">${displayLang}</span><button type="button" class="oc-copy-code" data-copy-code="${escapeHtml(codeText)}">复制</button>`;
...
function injectStyles() {
  ...
  style.innerHTML = `
    ...
    .oc-markdown { line-height:1.2; font-size:9px; word-break:break-word; }
    ...
    .oc-markdown pre { background:linear-gradient(135deg,#1e293b 0%,#0f172a 100%); color:#e2e8f0; border-radius:12px; padding:16px; overflow:auto; margin:12px 0; box-shadow:0 4px 16px rgba(0,0,0,.2); border:1px solid rgba(148,163,184,.1); }
    .oc-markdown pre code { background:transparent; padding:0; color:inherit; font-size:13px; line-height:1.6; }
    .oc-code-wrap { border-radius:12px; overflow:hidden; margin:12px 0; box-shadow:0 4px 16px rgba(0,0,0,.15); border:1px solid rgba(148,163,184,.15); }
    .oc-code-head { display:flex; justify-content:space-between; align-items:center; padding:10px 14px; background:linear-gradient(180deg,#1e293b 0%,#1a2332 100%); color:#94a3b8; font-size:12px; border-bottom:1px solid rgba(148,163,184,.1); }
    .oc-code-lang { font-weight:600; color:#cbd5e1; font-size:11px; text-transform:uppercase; letter-spacing:.5px; }
    .oc-copy-code { border:none; border-radius:6px; padding:6px 12px; cursor:pointer; background:rgba(59,130,246,.15); color:#60a5fa; font-size:11px; font-weight:600; transition:all .2s; }
    .oc-copy-code:hover { background:rgba(59,130,246,.25); color:#93c5fd; }
    .oc-copy-code:active { transform:scale(.95); }
    ...
    @media (min-width: 901px) {
      ...
      .oc-bubble{max-width:min(78%,760px);font-size:14px}
      .oc-markdown{font-size:14px;line-height:1.6}
      .oc-markdown code{font-size:13px}
      .oc-avatar{width:32px;height:32px;font-size:14px}
      .oc-title{font-size:14px}
      .oc-subtitle{font-size:12px;display:block}
      .oc-input{font-size:14px}
      .oc-send-btn{font-size:13px;height:36px;padding:0 16px}
      .oc-plus-btn{width:36px;height:36px;font-size:20px}
      .oc-field-inline{min-width:220px;flex:1}
      .oc-field-inline label{font-size:11px}
      .oc-field-inline .form-select{font-size:13px;height:34px}
      .oc-toolbar{flex-direction:row;flex-wrap:wrap;align-items:flex-end}
      .oc-toolbar-btn{font-size:12px;height:32px}
      .oc-session-item{font-size:13px;padding:12px 14px}
      .oc-new-chat{font-size:14px;padding:14px 16px}
      .oc-brand{font-size:20px}
      .oc-image-card-inline{width:min(180px,44vw)}
      .oc-image-card-inline img{max-height:180px}
      .oc-image-grid{grid-template-columns:repeat(auto-fit,minmax(220px,1fr))}
      .oc-file-chip{font-size:12px;padding:8px 12px}
    }
  `;
